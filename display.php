...
$ownerName = $contactNumber = $email = $vehicleModel = $vehicleNumber = $registrationDate = $vehicleType = "";
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    try {
        $stmt = $pdo->prepare("INSERT INTO reg (ownerName,contactNumber,email,vehicleModel, vehicleNumber, registrationDate, vehicleType)
                               VALUES (?, ?, ?, ?,?,?, ?)");
        $stmt->execute([$ownerName, $contactNumber,$email, $vehicleModel, $vehicleNumber, $registrationDate, $vehicleType]);
    } catch (PDOException $e) {
        if ($e->getCode() == 23000) {
